...
public class WagerStatsTable {
    ...
    public static void createTable() {
        try {
            ...
            Logger.getLogger(WagerStatsTable.class.getName()).log(Level.SEVERE, null, ex);          
        }
    }
    ...
    public static ResultSet selectAll() {
        try {
            Connection connection = DbConnect.connect();   
            Statement statement = connection.createStatement();
            String selectSQL = "(SELECT * FROM wagerStatistics)";
    ...
    public static void insertData (int column,int wagers,  int gameId, int drawId) {
        try {
            Connection connection = DbConnect.connect();
            String insertSQL = "INSERT INTO wagerStatistics "
                    + "(column, "
                    + "wagers, "
                    + "gameId, "
                    + "drawId) "
    ...
    public static void updateData (int column, int wagers ,  int gameId, int drawId) {
        try {
            Connection connection = DbConnect.connect();
            String updateSQL = "UPDATE wagerStatistics SET "
                    + "column = ?, "
                    + "wagers = ?, "
    ...
    public static void deleteTupple (int gameId, int drawId) {
        try {
            Connection connection = DbConnect.connect();
            String deleteSQL = "DELETE FROM wagerStatistics WHERE gameID = ? AND drawID = ?";
    ...
    public static void deleteGameData (int gameId) {
        try {
            Connection connection = DbConnect.connect();
            String deleteSQL = "DELETE FROM wagerStatistics WHERE gameID = ?";
            PreparedStatement preparedStatement = connection.prepareStatement(deleteSQL);
            preparedStatement.setInt(1, gameId);
            preparedStatement.executeUpdate();
            preparedStatement.close();
